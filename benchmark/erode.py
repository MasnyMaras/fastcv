...
def benchmark_erode(sizes=[1024, 2048, 4096], runs=50):
    results = []
    ...
    for size in sizes:
        ...
        for _ in range(runs):
            _ = cv2.erode(img_np, (3, 3), iterations=1)
        ...
        start = time.perf_counter()
        for _ in range(runs):
            _ = fastcv.erode(img_torch, 1) # doing 1 is half size of the filter ie (3 x 3)
        torch.cuda.synchronize()
        end = time.perf_counter()
